		...
		<form action="${pageContext.request.contextPath }/order/addOrder" name="form1" method="post">
			...
			<table width="1200" height="80" border="1" cellpadding="0" cellspacing="0" bordercolor="#d8d8d8">
				...
				<c:forEach items="${ cartList }" var="entry">
					<tr>
						<td><img src="${pageContext.request.contextPath }/${entry.product.imgurl}" width="90" height="90" /></td>
						<td>${ entry.product.name }</td>
						<td>${entry.product.price}</td>
						<td>${ entry.buyNum}件</td>
						<td>${ entry.product.price * entry.buyNum }元</td>
					</tr>
					<c:set var="totalMoney" value="${ totalMoney + entry.product.price * entry.buyNum }"/>
				</c:forEach>
			</table>
